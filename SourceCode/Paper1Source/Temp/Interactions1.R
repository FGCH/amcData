...
M3 <- coxph(Surv(year1980, AMCDecent) ~ SystemicCrisisLag3 + polariz*checks +
              cluster(imfcode) + strata(NumAMCCountryNoNA), data = Data)
...
## Decentralised
Sim2 <- coxsimInteract(M3, b1 = "polariz", b2 = "checks", qi = "Marginal Effect", X2 = c(1:7))
gginteract(Sim2, qi = "Marginal Effect", smoother = "lm",
           ylab = "Marginal Effect of Polarization\n",
           xlab = "\nChecks")
